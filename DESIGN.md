...
- **Loading:** Slides carry `data-bg` rather than a background image. The visible slide is fetched at once; neighbours wait for idle, and on arrival only the *next* one is queued — the slide before the first is the last of thirteen, and nobody clicks backwards on landing. Viewports at or below `700px` are served the `img/small/` copies (1000px wide); wider ones get `img/` at 1920px, which is 2x the 960px band. Neither tier is larger than the band can show. The first slide keeps a small inline background so the band still renders without JS.
